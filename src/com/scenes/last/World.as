package com.scenes.last
{
	...
	import com.scenes.scene2.General.Input;
	...
	public class World extends MovieClip
	{
		...
		protected const SWF_HALF_WIDTH:int = 400;
		protected const SWF_HEIGHT:int = 600;
		...
		private function drawBorder(xx:uint,yy:uint,wdt:uint,hgt:uint):void{
			var borderDef:b2BodyDef = new b2BodyDef();
			borderDef.position.Set(xx/PIXELS_TO_METRE,yy/PIXELS_TO_METRE);
			var borderBody:b2Body = _world.CreateBody(borderDef);
			var shp:b2PolygonShape = new b2PolygonShape();
			shp.SetAsBox(wdt/PIXELS_TO_METRE,hgt/PIXELS_TO_METRE);
			
			var groundFixtureDef:b2FixtureDef = new b2FixtureDef();
			groundFixtureDef.shape = shp;
			groundFixtureDef.density = 1;
			groundFixtureDef.friction = 1;
			borderBody.CreateFixture(groundFixtureDef);
		}
		
		protected function init():void
		{
			_world = new b2World(new b2Vec2(0, 0), true);
			drawBorder(0,0,Dimentions.WIDTH,1);
			drawBorder(Dimentions.WIDTH,0,1,Dimentions.HEIGHT);
			var groundBodyDef:b2BodyDef = new b2BodyDef();
			groundBodyDef.position.Set(SWF_HALF_WIDTH / PIXELS_TO_METRE, Dimentions.HEIGHT / PIXELS_TO_METRE);
			
			_groundBody = _world.CreateBody(groundBodyDef);
			
			var groundBox:b2PolygonShape = new b2PolygonShape();
			groundBox.SetAsBox(Dimentions.WIDTH / PIXELS_TO_METRE, 10 / PIXELS_TO_METRE);
